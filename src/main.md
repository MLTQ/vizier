...
- **Rationale**: Bare `vz` defaults to a one-shot compact `snapshot` for quick terminal use; `wake` remains explicit and uses compact output unless `--verbose` is set.
...
|-----------|---------|------------------|
| CLI users | Bare `vz` emits a one-shot compact snapshot by default; `vz wake`, `vz snapshot`, `vz watch` remain available | Default command behavior, command names/flags, output format |
